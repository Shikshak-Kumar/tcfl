...
class MockTrafficEnvironment:
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        for edge in green_edges:
            ...
            if actual_flux > 0 and self.neighbors:
                for _ in range(actual_flux):
                    if (
                        random.random() < 0.7
                    ):  # 70% chance to enter a neighbor intersection
        ...
        reward = max(-10.0, min(1.0, reward))  # Tight clip for stability
            ...
            self.total_vehicles >= self.target_vehicles
            and sum(self.lane_queues.values()) == 0
        )
        ...
        if self.show_phase_console and self.step_count % 10 == 0:
            print(
                f"[Mock] Step {self.step_count}: Queues {self.lane_queues}, Accidents {self.total_accidents}, Reward {reward:.2f}"
            )
    ...
    def get_performance_metrics(self) -> Dict:
        return {
            ...
            "avg_waiting_time_per_vehicle": (
                self.total_waiting_time / max(1, self.total_vehicles)
            ),
    ...
    def _compute_lane_summary(self) -> Dict:
        """Compute lane-level summary metrics from internal state."""
        queues = list(self.lane_queues.values())
        num_lanes = len(self.incoming_edges)

        # Use cumulative total (same as get_performance_metrics) — never resets to 0
        total_wait = self.total_waiting_time

        # Use average queue length over the episode, not the instantaneous end snapshot
        avg_queue = (
            float(np.mean(self.queue_lengths))
            if self.queue_lengths
            else float(sum(queues))
        )
        max_queue = float(max(self.queue_lengths)) if self.queue_lengths else 0.0

        # A lane is "congested" if current queue > 5
        num_congested = sum(1 for q in queues if q > 5)

        return {
            "total_waiting_time": total_wait,
            "total_queue_length": avg_queue,
            "num_congested_lanes": float(num_congested),
            "total_lanes": float(num_lanes),
            "average_gst": 0.0,
            "min_gst": 0.0,
            "max_gst": 0.0,
            "max_queue_length": max_queue,
            "max_waiting_time": total_wait,
            "average_speed": 0.0,
            "num_active_lanes": float(sum(1 for q in queues if q > 0)),
            "total_occupancy": min(100.0, avg_queue * 5.0),
            "total_vehicles": self.total_vehicles,
        }
    ...
    def _generate_detailed_metrics(self) -> Dict:
        ...
        for edge in self.incoming_edges:
            ...
            detailed[lane_id] = {
                ...
                "average_speed": random.uniform(0, 15)
                if self.lane_queues[edge] == 0

...
class SUMOTrafficEnvironment:
    def __init__(
        ...
        self.phases = {"GGrrGGrr", "yyrryyrr", "rrGGrrGG", "rryyrryy"}
                    ...
                    for conn in conn_group:
                        if conn and len(conn) >= 1:
                            ...
                            try:
                                ...
                                if (
                                    edge_id in self.incoming_edges
                                    and edge_id not in self._edge_to_lane
                                ):
    ...
    def _compute_green_signal_times(self) -> Dict:
        try:
            ...
            avg_gst = (
                float(np.mean(list(gst_per_edge.values()))) if gst_per_edge else 0.0
            )
            snapshot = {
    ...
    def _setup_gst_gui_pois(self):
        ...
        try:
            for edge_id in self.incoming_edges:
                ...
                try:
                    traci.poi.add(
                        poi_id, x, y, (0, 255, 0, 255), name=f"GST {edge_id}: --s"
                    )
                except Exception:
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, Dict]:
        try:
            ...
            try:
                ...
                self.total_arrived += arrived_now
                print("ARRIVED THIS STEP:", arrived_now)
                print("TOTAL ARRIVED:", self.total_arrived)
            except Exception:
                pass
            try:
                departed_now = traci.simulation.getDepartedNumber()
                self.total_departed += departed_now
                print("DEPARTED THIS STEP:", departed_now)
                print("TOTAL DEPARTED:", self.total_departed)
            except Exception:
                pass
            try:
            ...
            if self.show_phase_console and phase_info:
                print(
                    f"[TLS {self.tl_id}] phase={phase_info.get('phase')} remaining={phase_info.get('remaining_s'):.1f}s/{phase_info.get('duration_s'):.1f}s"
                )
                        ...
                        if extension > 0.5:
                            try:
                                traci.trafficlight.setPhaseDuration(
                                    self.tl_id, remaining + extension
                                )
                            except Exception:
                                pass
            except Exception:
        ...
        try:
            ...
            if self._pending_target_phase is not None:
                ...
                if remaining <= 0.1:
                    ...
                    try:
                        traci.trafficlight.setPhaseDuration(
                            self.tl_id, self.min_phase_duration_s
                        )
                    except Exception:
                        pass
            ...
            if (
                desired != current
                and (sim_time - self.last_phase_switch_time)
                >= self.min_phase_duration_s
            ):
                if is_green(current) and is_green(desired) and current != desired:
                    ...
                    if y is not None:
                        ...
                        try:
                            traci.trafficlight.setPhaseDuration(
                                self.tl_id, self.yellow_duration_s
                            )
                        except Exception:
                            pass
                        self.last_phase_switch_time = sim_time
    ...
    def _get_detailed_lane_metrics(self) -> Dict:
        try:
            detailed = {"per_lane_metrics": {}, "lane_summary": {}}
            ...
            for edge_id in self.incoming_edges:
                ...
                try:
                    ...
                    lane_ids = [
                        ...
                        if traci.lane.getEdgeID(lid) == real_edge
                    ]
                except Exception:
                    lane_ids = []
            ...
            if lanes:
                detailed["lane_summary"] = {
                    ...
                    "average_speed": float(
                        np.mean([l["average_speed"] for l in lanes])
                    ),
                    "average_occupancy": float(
                        np.mean([l["occupancy_percent"] for l in lanes])
                    ),
                    "num_congested_lanes": len(
                        [l for l in lanes if l["queue_length"] > 3]
                    ),
                }
    ...
    def get_performance_metrics(self) -> Dict:
        metrics = {
            ...
            "throughput_ratio": (self.total_arrived / max(1, self.total_departed)),
            "avg_waiting_time_per_vehicle": float(
                self.total_waiting_time / max(1, self.total_arrived)
            ),
            # "total_vehicles": self.total_vehicles,
        ...
        try:
            if self._per_vehicle_wait:
                ...
                metrics["avg_waiting_time_per_vehicle"] = total_per_vehicle_wait / max(
                    1, num_vehicles
                )
            else:
        ...
        try:
            ...
            avg_per_edge = {
                e: (per_edge_sums[e] / max(1, per_edge_counts[e]))
                for e in per_edge_sums
            }

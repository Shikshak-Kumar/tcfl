...
def run_single_client_training(
    ...
    for round_num in range(num_rounds):
        ...
        print(
            f"  Avg Waiting Time per Vehicle (episode): {eval_metrics[2].get('avg_waiting_time_per_vehicle', 0):.2f}s"
        )
        ...
        if per_lane_metrics:
            ...
            for edge_id, lane_data in per_lane_metrics.items():
                ...
                if vehicle_types:
                    type_str = ", ".join(
                        [f"{vtype}: {count}" for vtype, count in vehicle_types.items()]
                    )
        ...
        if lane_summary:
            ...
            print(
                f"  Total Waiting Time: {lane_summary.get('total_waiting_time', 0):.2f}s"
            )
    ...
    client.save_training_history(
        os.path.join(results_dir, "single_client_training.json")
    )
...
def run_multi_client_simulation(
    results_dir: str = "results_federated",
    gui: bool = False,
    num_rounds: int = 15,
    num_clients: int = 2,
    ...
    for round_num in range(num_rounds):
        ...
        for client in clients:
            current_params = (
                ...
                else client.get_parameters({})
            )

            config = {"round": round_num, "episodes": 3, "learning_rate": 0.001}
        ...
        for client, params in zip(clients, params_list):
            ...
            norm_cong = num_congested / max(
                total_lanes, 1.0
            )  # fraction of congested lanes
        ...
        avg_waiting_time = np.mean(
            [m.get("waiting_time", 0) for m in eval_metrics_list]
        )
...
def main():
    parser = argparse.ArgumentParser(description="Federated Traffic RL")

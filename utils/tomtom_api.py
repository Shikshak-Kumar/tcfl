...
def get_api_key() -> str:
    ...
    return os.environ.get("TOMTOM_API_KEY", "oK2pgm45ieRxyEPgv876db2lGarwDFm2") # Fallback for now if not set

def get_real_time_flow(api_key: str, lat: float, lon: float) -> Optional[Dict[str, float]]:
    """
    ...
    try:
        ...
        with urllib.request.urlopen(req, context=context, timeout=10) as response:
            ...
            if 'flowSegmentData' in data:
                ...
                print(f"Warning: 'flowSegmentData' not found in TomTom response for {lat},{lon}")
                return None
...
def get_incidents(api_key: str, lat: float, lon: float, radius_deg: float = 0.05) -> List[Dict]:
    """
    ...
    url = (f"https://api.tomtom.com/maps/orbis/traffic/incidentDetails?"

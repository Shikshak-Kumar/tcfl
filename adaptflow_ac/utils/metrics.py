...
class MetricsTracker:
    ...
    def update(self, node_id, metrics, reward):
        """
        metrics: dict containing 'avg_waiting', 'avg_queue', 'throughput', 'congested_lanes'
        ...
        self.history[node_id]["throughput"].append(metrics.get("throughput", 0))
    ...
    def print_table(self):
        ...
        print("\n" + "="*80)
        print(f"{'Node ID':<15} | {'Wait (s)':<10} | {'Queue':<10} | {'TP':<10} | {'Congest':<10} | {'Reward':<10}")
        print("-" * 80)
        for nid in self.node_ids:

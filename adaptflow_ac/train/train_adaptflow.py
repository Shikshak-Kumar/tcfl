...
from train.fedflow_cluster import FedFlowCluster
from train.fedflow_server import FedFlowServer
from train.adaptive_clustering import (
...
class AdaptFlowTrainer:
    ...
    def _setup_environments(self):
        ...
        if not self.gui:
            if self.use_tomtom:
                ...
                for i in range(self.num_nodes):
                    config = self.sumo_configs[i % len(self.sumo_configs)]
                    self.envs[f"node_{i}"] = MockTrafficEnvironment(num_intersections=1, config=config)
                    print(f"  node_{i} -> {config}")
    ...
    def _infer_results_mode_labels(self) -> Tuple[str, str]:
        ...
        try:
            from env.tomtom_traffic_environment import TomTomTrafficEnvironment
            is_tomtom = isinstance(e0, TomTomTrafficEnvironment)
        except ImportError:
            is_tomtom = False
    ...
    def run_round(self, round_idx: int):
        ...
        total_steps = self.steps * 10 # Assuming 10 rounds for annealing reference
        global_step_offset = (round_idx - 1) * self.steps
        ...
        for nid, agent in self.agents.items():
            ...
            # Warm-up phase: fill buffer with random policy if empty
            if agent.memory.tree.n_entries < agent.min_buffer_size:
                print(f"    {nid}: Warm-up phase (filling buffer {agent.memory.tree.n_entries}/{agent.min_buffer_size})...")
                while agent.memory.tree.n_entries < agent.min_buffer_size:
                    state_graph, adj_node = self._get_node_graph_state(nid, state)
                    state_seq = agent._get_sequence(state_graph)
                    # Random actions
                    action = [np.random.randint(agent.action_dim) for _ in range(len(state_graph))]
                    next_state, reward, done, _ = env.step([action[0]])
                    # Handle rewards (list or dict)
                    if isinstance(reward, dict):
                        local_reward = reward.get(nid, list(reward.values())[0])
                    elif isinstance(reward, (list, np.ndarray)):
                        local_reward = reward[0] # Single-node env
                    else:
                        local_reward = reward
                    ...
                    agent.remember(state_seq, adj_node, action, [local_reward], next_state_seq, next_adj_node, done)
                    state = next_state
                    if done: state = env.reset()
                print(f"    {nid}: Warm-up complete.")

            for t in range(self.steps):
                # 1. Annealing
                current_global_step = global_step_offset + t
                # β_entropy: 0.05 -> 0.001
                agent.beta = max(0.001, 0.05 - (0.05 - 0.001) * (current_global_step / total_steps))
                # β_per: 0.4 -> 1.0
                beta_per = min(1.0, 0.4 + (1.0 - 0.4) * (current_global_step / total_steps))
                # LR decay: lr_init * max(0.1, 1.0 - global_step / max_steps)
                lr_factor = max(0.1, 1.0 - current_global_step / total_steps)
                for param_group in agent.actor_optimizer.param_groups:
                    param_group['lr'] = 5e-4 * lr_factor
                for param_group in agent.critic_optimizer.param_groups:
                    param_group['lr'] = 2.5e-4 * lr_factor
                ...
                # Extract rewards for all nodes in the neighborhood
                # In this mock/SUMO setup, we'll use local_reward for index 0 
                # and small random rewards for neighbors if not available
                if isinstance(reward, dict):
                    local_reward = reward.get(nid, list(reward.values())[0])
                elif isinstance(reward, (list, np.ndarray)):
                    ...
                    local_reward = reward
                all_rewards = [local_reward] + [local_reward * 0.5] * (len(state_graph) - 1)
            ...
            node_losses[nid] = loss if 'loss' in locals() else 0.0

            print(f"    {nid}: Reward={total_reward:.1f}, AvgWait={metrics.get('avg_waiting_time_per_vehicle', 0):.2f}s, Loss={node_losses[nid]:.4f}")
    ...
    def train(self, num_rounds: int = 3):
        ...
        try:
            ...
            ModelExporter.export(agent.model, metadata, save_path)
